...
# # app/crud/payment.py - COMPLETELY FIXED
# from sqlalchemy.orm import Session
# from app.models import Payment, RentCycle, Tenant, Room, Property, Landlord
# from app.schemas import PaymentCreate
# from datetime import datetime

# def create_payment(db: Session, payment: PaymentCreate, tenant_id: int):
...
# def get_payment_history(db: Session, tenant_id: int):
#     """Get payment history for a tenant with all required fields"""
...
#     ).join(
#         Landlord, Property.landlord_id == Landlord.landlord_id
#     ).filter(
#         Payment.tenant_id == tenant_id
#     ).order_by(Payment.payment_date.desc()).all()
...
#         landlord = None
#         if rent_cycle:
#             room = db.query(Room).join(Property).filter(Room.room_id == rent_cycle.room_id).first()
#             if room and room.property:
...
#             "landlord_contact": landlord.phone_number if landlord else "N/A",
#             "remaining_balance": tenant.balance if tenant else 0.0
#         }
#         result.append(payment_dict)
...
# def get_payment_by_id(db: Session, payment_id: int):
#     """Get a specific payment with all details"""
#     payment = db.query(Payment).filter(Payment.payment_id == payment_id).first()
#     if not payment:
#         return None
