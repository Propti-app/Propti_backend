...
from app.models import Payment, RentCycle, Tenant, Room, Property, Landlord, PaymentStatus
...
def create_payment(db: Session, payment: PaymentCreate):
    """
    Create a payment, update balances/statuses, and return an enriched dict.
    Flutter receipt screen uses this response directly — no second API call needed.
    """
    db_payment = Payment(
    ...
    if rent_cycle:
        existing_paid = sum(p.amount for p in rent_cycle.payments)
        total_paid = existing_paid + payment.amount
        ...
        if room:
            room.payment_status = rent_cycle.payment_status
    ...
    return _enrich(db_payment, tenant, room, property_obj, landlord)
...
def get_payment_by_id(db: Session, payment_id: int):
    ...
    return _enrich(payment, tenant, room, property_obj, landlord)
...
def _enrich(payment, tenant, room, property_obj, landlord):
    return {
        ...
        # Joined — used by Flutter receipt screen
