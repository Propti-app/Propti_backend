...
#     ).filter(
#         models.Payment.payment_id == payment_id,
#         models.Room.property.landlord_id == landlord_id
#     ).first()
#     if not payment:
#         raise HTTPException(status_code=404, detail="Payment not found")
...
#     db: Session = Depends(get_db)
# ):
#     # Verify rent cycle belongs to landlord
#     rent_cycle = db.query(models.RentCycle).join(
#         models.Room, models.RentCycle.room_id == models.Room.room_id
#     ).join(
#         models.Property, models.Room.property_id == models.Property.property_id
#     ).filter(
...
#     ).first()
#     if not rent_cycle:
#         raise HTTPException(status_code=403, detail="Not authorized to record payment for this rent cycle")
#     # Verify tenant is assigned to the room
#     tenant = db.query(models.Tenant).filter(
#         models.Tenant.tenant_id == payment.tenant_id,
...
#     db_payment = crud.create_payment(db, payment)
#     if not db_payment:
#         raise HTTPException(status_code=404, detail="Rent cycle or tenant not found")
#     return db_payment