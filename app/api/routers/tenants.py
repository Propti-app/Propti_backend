...
# ):
#     # Verify room belongs to landlord
#     room = db.query(crud.models.Room).join(crud.models.Property).filter(
#         crud.models.Room.room_id == room_id,
...
#     db: Session = Depends(get_db)
# ):
#     tenant = db.query(crud.models.Tenant).filter(crud.models.Tenant.tenant_id == tenant_id).first()
#     if not tenant:
#         raise HTTPException(status_code=404, detail="Tenant not found")
#     # Verify tenant's room (if assigned) belongs to landlord
#     if tenant.assigned_room_id:
#         room = db.query(crud.models.Room).join(crud.models.Property).filter(
#             crud.models.Room.room_id == tenant.assigned_room_id,
#             crud.models.Property.landlord_id == landlord_id
#         ).first()
#         if not room:
#             raise HTTPException(status_code=403, detail="Not authorized to vacate this tenant")
